...
def sample_essential_domain(func,u=None,eps_perturbation=None):
    ...
    if not func.separable:
        raise NotImplementedError('Only implemented for separable functionals')
    if isinstance(func.domain,MeasureSpaceFcts):
        ...
        if not isinstance(func, (FunctionalOnDirectSum,Conj,HorizontalShiftDilation)):
            raise TypeError(f"domain of type {func.domain}, but functional of type {type(func)},{func}.")
...
def test_subgradient(func,u=None,h_length=1e-8,tol_smooth=1e-2,tol_convex=1e-3):
    ...
    err= diffq+deriv
    if err>tol_convex*func.h_domain.norm(grad_u):
        raise RuntimeError(f'Convexity test failed: err={err}, tol_convex={tol_convex},norm(grad_u)={func.h_domain.norm(grad_u)}')
    if np.abs(err)>tol_smooth*func.h_domain.norm(grad_u):
        raise RuntimeError(f'Smoothness test falied: err={err}, tol_smooth={tol_smooth}, |grad|={func.h_domain.norm(grad_u)}.')
...
def test_second_derivative(func,u=None,h=None,eps=1e-8,tolerance = 1e-2,abs_tol=1e-6):
    ...
    assert func.h_domain.norm(func_pp-diffq)<=np.max([abs_tol,tolerance * func.h_domain.norm(func_pp)]), f"err: {err}, tol: {tolerance}, norm second deriv. {func.h_domain.norm(func_pp)}"
...
def test_Lipschitz_convexity(func,u=None,safety=1.5):
    ...
    assert func.Lipschitz>=(1-1e-10)*ub, f"Lipschitz constant {func.Lipschitz} is smaller than second derivative {np.max(fpp)}" 
    ...
    assert func.convexity_param<=lb+1e-12, f"Convexity parameter {func.convexity_param} is larger than second derivative {lb}"
