...
from regpy.operators import Operator, Multiplication, RealPart, SquaredModulus
...
class myExponential(Operator):
    ...
    def _eval(self, x, differentiate=False):
        ...
        arg_g = x.imag
        if differentiate:
            self._factor_abs_g = np.exp(1j*arg_g)
            self._factor_arg_g = 1j*abs_g*np.exp(1j*arg_g)
        return abs_g*np.exp(1j*arg_g)

    def _derivative(self, x):
        #abs_x, arg_x = self.domain.split(x)
        abs_g = x.real 
        arg_g = x.imag
        return self._factor_abs_g * abs_g + self._factor_arg_g * arg_g
    ...
    def _adjoint(self, y):
        abs_res = self._factor_abs_g.real * y.real + self._factor_abs_g.imag * y.imag
        arg_res = self._factor_arg_g.real * y.real + self._factor_arg_g.imag * y.imag
        return abs_res+1j*arg_res #self.domain.join(abs_res,arg_res)
...
def PINEM_g_to_data(domain, fresnel_number,N=1):
    assert not domain.is_complex
    cdomain = domain.complex_space()
    op_list = []
    for n in range(-N,N+1):
        if not n==0:
            op_list.append(fresnel_propagator(cdomain, n*fresnel_number)*Nemitzky_op_for_g(cdomain,n))
