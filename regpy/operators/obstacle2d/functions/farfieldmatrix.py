...
def farfield_matrix(bd,dire,kappa,weight_sl,weightDL):
    """set up matrix corresponding to the far field evaluation of a combined
    %single and double layer potential with weigths weight_sl and weightDL, rsp."""
#    print(dire)
    FFmat =  np.pi / (np.size(bd.z,1)*np.sqrt(8*np.pi*kappa)) * np.exp(-complex(0,1)*np.pi/4) \
         * (weightDL*kappa*dire.T.dot(bd.normal) +complex(0,1)*weight_sl*np.matlib.repmat(bd.zpabs,np.size(dire,1),1)) \
         * np.exp(-complex(0,1)*kappa* (dire.T.dot(bd.z)))
    return FFmat


def farfield_matrix_trans(bd,dire,kappa,weight_sl,weightDL):
    """set up matrix corresponding to the far field evaluation of a combined
    %single and double layer potential with weigths weight_sl and weightDL, rsp.
