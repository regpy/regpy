...
class OperatorNode:
    ...
    def get_node_from_operator(op):
        if(isinstance(op,PartOfOperator)):
            ...
            node.output_edges.append(Edge(out_adress,OperatorAdressNull))
            return node
        if(isinstance(op,Operator)):
            node=OperatorNode(op)
            node.output_edges.append(Edge(OperatorAdressAll(node,False),OperatorAdressNull))
...
class Edge:
    ...
    def __init__(self,start_adress,end_adress):
        assert self.start_adress.is_input_adress==False
        assert self.start_adress.is_input_adress
...
class OperatorGraph(Operator):
    ...
    def __init__(self, domain=None, codomain=None, linear=False):
        super().__init__(domain, codomain, linear)
