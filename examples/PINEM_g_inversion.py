...
def main():
    ...
    # If true, abs(g) is assumed to be known on nanotip.
    ...
    if g_is_from_simulation:
        op, grid, exact_solution, g_map, mask_a, mask_p \
            = setup_simulated_g(g_is_complex=g_is_complex, 
                                ...
                                list_of_filters= [ [6],[7],[8],[10],[15] ],
                                #[ [1,3,5,7],[-1,-3,-5,-7]],
                                #N=8,
                                parallel=True
                                )
        # amplitude of g assumed to be known everywhere
        mask_a = np.full(mask_a.shape,False,dtype=bool)
        sobolev_index = 1
        if g_is_complex:
            regpar = 1e-4; regpar_step = 1/2
        else:
            regpar = 1e-4; regpar_step = 1/2
    else:
        op, grid, exact_solution, g_map, mask_a, mask_p \
    ...
    if g_is_complex:
        ...
        if abs_g_is_partly_known:
            ...
            ampl_domain = Hm0_domain(mask_p, index = sobolev_index)
    ...
    ############################### compute synthetic data and define Hilbert space setting
    ...
    if using_g_squared_measurement:
        ...
        Hcodomain0 = L2(grid, weights = 1/(scal_EELS**2 + scal_EELS*exact_data_comp[0]))
    ...
    stoprule = (
        ...
        rules.CountIterations(max_iterations=0,while_type=True) 
    )
#    solver = NewtonCG(
...
#    )
    ...
    def plot_reco(fig1,fig2,reco_amp,reco_phase,reco_data_comp,g_map,ex_data_comp,Newton_step):
        ...
        plotdata.append({'pos': (1, 0), 'data': reco_amp.T,
                        'title': 'reco |g| it.{}'.format(Newton_step)})
