...
n_iter   = 2e5
stepsize = [1e-2, 1e-1, 5e-1, 7e-1, 1e0, 1.2, 1.5, 2.5, 10, 20][0]
Temperature=1e-5
reg_parameter=1e-4
...
plot_mean(statemanager, exact_solution, n_iter=1500)
...
bd=op.bd
pts=bd.coeff2Curve(m+3*v, n)
pts_2=bd.coeff2Curve(exact_solution, n)
pts_3=bd.coeff2Curve(m-3*v, n)
...
pts_2=bd.coeff2Curve(exact_solution, n)
pts_3=bd.coeff2Curve(m-3*v, n)
poly = Polygon(np.column_stack([pts[0, :], pts[1, :]]), label='mean +3*variance',  animated=True, fill=False)
poly_2=Polygon(np.column_stack([pts_2[0, :], pts_2[1, :]]), label='exact_solution', animated=True, fill=False)
poly_3=Polygon(np.column_stack([pts_3[0, :], pts_3[1, :]]), label='mean-3*variance', animated=True, fill=False)
