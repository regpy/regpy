...
class gaussian(object):
    ...
    def gaussian(self, x):
        ...
        return -1 / 2 * np.dot(
            (x - self.m_0).reshape(self.len_domain),
            np.dot(self.gamma_prior_inv, np.conjugate((x - self.m_0).reshape(self.len_domain)))
        ).real
...
class tikhonov(object):
    ...
    def tikhonov(self, x):
        return -0.5 * self.regpar * self.setting.Hdomain.inner(x, x)
    ...
    def gradient_tikhonov(self, x):
        return -self.regpar * self.setting.Hdomain.gram(x)
