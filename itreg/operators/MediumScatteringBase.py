...
class MediumScatteringBase:
    ...
    # in space domain on fine grid
        v_mat=np.zeros(params.scattering.N)
        v_mat = np.reshape(v,params.scattering.N, order='F')
        v_mat = np.fft.ifftn(params.scattering.prec.K_hat * np.fft.fftn(v_mat))
        return v + data.contrast * v_mat.reshape(np.prod(params.scattering.N), order='F')

    def AdjointLippmannSchwingerOp(params, data, v):
    #adjoint of LippmannSchwingerOp
        aux=np.zeros(params.scattering.N)
        aux = np.reshape(np.conj(data.contrast)*v,params.scattering.N, order='F')
        aux = np.fft.ifftn(np.conj(params.scattering.prec.K_hat) * np.fft.fftn(aux))
        return v + aux.reshape(np.prod(params.scattering.N), order='F')
