...
class ReactionCoefficient(NonlinearOperator):
    ...
    class operator(OperatorImplementation):
        def eval(self, params, c, data, differentiate, **kwargs):
            ...
            res=np.zeros(params.domain.shape)
            for i in range(0, params.domain.shape):
...
def B_builder(params, c):
    B=np.zeros((params.domain.shape, params.domain.shape))
    for i in range(0, params.domain.shape):
        for j in range(0, params.domain.shape):
